...
import kr.kro.tripsketch.commons.utils.HighlightLoggerWrapper
...
class AuthService(
    ...
    private val logger = HighlightLoggerWrapper(LoggerFactory.getLogger(OauthController::class.java))
    /**
    ...
    fun refreshUserToken(request: KakaoRefreshRequest): TokenResponse? {
        ...
        if (newKakaoRefreshToken != null) {
            logger.warn("[$timestamp] 4. 카카오 리프레시 토큰 업데이트")
            userService.updateKakaoRefreshToken(user.memberId, newKakaoRefreshToken)
        }

        logger.warn("[$timestamp] 5. 유저 마지막 로그인 업데이트")
        user.updateLastLogin()

        logger.warn("[$timestamp] 6. JWT 토큰 생성 중")
        val tokenResponse = jwtService.createTokens(user)

        user.ourRefreshToken = tokenResponse.refreshToken
        userService.saveOrUpdate(user)
        logger.warn("[$timestamp] 7. 유저 정보 업데이트 완료: memberId=${user.memberId}")
